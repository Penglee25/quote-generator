import axios from "axios";

const API_URL = "https://quote-garden.onrender.com/";
const API_VERSION = "api/v3/";

export const randomQuote = (): Promise<any> => {
  return new Promise((success, reject) => {
    axios
      .get(`${API_URL}${API_VERSION}quotes/random`)
      .then(({ data }) => {
        success(data);
      })
      .catch((error) => {
        reject(error);
      });
  });
};

export const authorQuotes = (authorName: string, page = 1, limit = 10): Promise<any> => {
  return new Promise((success, reject) => {
    if (authorName === undefined) {
      reject({ error: "Author is required" });
    }
    axios
      .get(
        `${API_URL}${API_VERSION}authors/${authorName}?page=${page}&limit=${limit}`
      )
      .then(({ data }) => {
        success(data);
      })
      .catch((error) => {
        reject(error);
      });
  });
};

export const genreQuotes = (genre: string, page = 1, limit = 10): Promise<any> => {
  return new Promise((success, reject) => {
    if (genre === undefined) {
      reject({ error: "Genre is required" });
    }
    axios
      .get(`${API_URL}${API_VERSION}genre/${genre}?page=${page}&limit=${limit}`)
      .then(({ data }) => {
        success(data);
      })
      .catch((error) => {
        reject(error);
      });
  });
};

export const getQuotes = (page = 1, limit = 10): Promise<any> => {
  return new Promise((success, reject) => {
    axios
      .get(`${API_URL}${API_VERSION}quotes?page=${page}&limit=${limit}`)
      .then(({ data }) => {
        success(data);
      })
      .catch((error) => {
        reject(error);
      });
  });
};

export const searchQuotes = (searchKeyword: string, page = 1, limit = 10): Promise<any> => {
  return new Promise((success, reject) => {
    if (searchKeyword === undefined) {
      reject({ error: "Search keyword required" });
    }
    axios
      .get(
        `${API_URL}${API_VERSION}quotes/${searchKeyword}?page=${page}&limit=${limit}`
      )
      .then(({ data }) => {
        success(data);
      })
      .catch((error) => {
        reject(error);
      });
  });
};